  ...
  (function() {
    ...
    var loadPackage = function(metadata) {
      ...
      function fetchChunk(partName, partIndex, totalParts, callback, errback) {
        var url = Module['locateFile'] ? Module['locateFile'](partName, '') : partName;
        ...
        xhr.onprogress = function(event) {
          ...
          if (!xhr.addedTotal) {
            xhr.addedTotal = true;
            Module.dataFileDownloads[partName] = { loaded: 0, total: REMOTE_PACKAGE_SIZE / totalParts };
          }
          if (event.loaded) {
            Module.dataFileDownloads[partName].loaded = event.loaded;
          }
          var total = 0, loaded = 0, num = 0;
          for (var d in Module.dataFileDownloads) {
            total += Module.dataFileDownloads[d].total;
            loaded += Module.dataFileDownloads[d].loaded;
            num++;
          }
          total = Math.ceil(total * Module.expectedDataFileDownloads / num);
          if (Module['setStatus']) Module['setStatus']('Downloading data... (' + loaded + '/' + total + ')');
        };
        xhr.onerror = function() { errback(new Error("NetworkError for: " + url)); };
        xhr.onload = function() {
      ...
      function fetchAllChunks(callback, errback) {
        var results = new Array(CHUNK_PARTS.length);
        var remaining = CHUNK_PARTS.length;
        CHUNK_PARTS.forEach(function(partName, i) {
          fetchChunk(partName, i, CHUNK_PARTS.length, function(data) {
            results[i] = data;
            remaining--;
            if (remaining === 0) {
              // Combine all chunks into one ArrayBuffer
              var totalSize = results.reduce(function(sum, buf) { return sum + buf.byteLength; }, 0);
              var combined = new Uint8Array(totalSize);
              var offset = 0;
              results.forEach(function(buf) {
    ...
    function runWithFS() {
      ...
      // Fetch all chunks, combine, then process
      fetchAllChunks(function(combinedBuffer) {
        processPackageData(combinedBuffer);
      }, handleError);
